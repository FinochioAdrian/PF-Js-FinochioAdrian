...
function renderizarCarro() {
    ...
    let template = ""
    carroCompras.length<1? productosCarrito.innerHTML=` <!-- producto en carrito -->
<div class="card mb-3">
    ...
    <div class="card mb-3">
      <div class="row g-0 align-items-center cart__producto">
        ...
        <div class="col-8 col-md-8 cart__details">
          <div class="card-body">
            ...
            <div class="producto__resumenPrice d-flex flex-column flex-md-row">
              <p class="col-md-4">
                Precio: $<span class="producto__precio">${producto.precio}</span>
              </p>
              <p class="col-md-4">
                Cantidad: <span class="producto__cantidad">${producto.cantidad}</span>
              </p>
              <p class="col-md-4">
                Precio Total: $<span class="producto__precio">${producto.precio*producto.cantidad}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="col-1 col-md-2 cart__trashContainer">
          <i class="bi bi-trash-fill  btn-borrar btn-borrar-producto" id="${producto.id}-borrar"></i>
...
:   cartResumen.innerHTML=`<p class="card-text ms-3">Producto (${carroCompras.length}) <span class="cart__subtotalTotal" >${subtotal=carroCompras.reduce((acu, producto,) => {return acu+(producto.cantidad*producto.precio)}, 0)}</span></p>
<p class="card-text ms-3">Envio  <span class="cart__envio" >consultar </span></p>
<p class="card-text ms-3">Total  $<span class="cart__Total" >${subtotal} </span></p>
<div class="d-flex justify-content-center">
<button class="btn btn-success procesar-pago" id="2">
